...
async function getDetail (page, url, id, indexUrl) {
  ...
  return await page.evaluate((indexUrl) => {
    //const subjectElemets = document.querySelector(`#${id}`);
    const datas = Array.from(document.querySelectorAll('.sytab')).map((data, index) => {
      ...
      return {
        ...
        corseFor: detailData[1],
        credets: detailData[2],
...
!(async() => {
  try {
    ...
    let toJsonArray = dataList.map((el, elIndex) => {
        ...
        return {
          SubjectCategoryName: el.SubjectFieldName,
          SubjectCategoryId: ('000' + (elIndex)).slice(-2),
